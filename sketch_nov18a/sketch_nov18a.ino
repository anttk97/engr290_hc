...
#define DELAY_ms(x)   TIMSK1&=~(1<<ICIE1);\
                      delay_ms=0;\
                      TIMSK1|=(1<<ICIE1);\
                      while (delay_ms<=x)
...
// Averaging filter
// If you use an ultrasonic sensor (US), this value MUST be set to 1 as per the sensor's manufacturer application notes.
...
// Note 4: The ADC values are printed in the following order:
// ADC5, ADC4, ADC3.
// Note 5: Since unconnected ADC inputs are floating, the displayed values of unconnected channels are quasi-random.
//***************************************************************************************************************

static volatile uint8_t msg_char=1, rx_buff, buttons, V_batt=255, ADC_sample, ctrl[5], ctrl_count, checksum;
...
//Now, TCNT1 will increment every 20ms/2500=8us. You can use it to measure the width of the
//US sensor response.

//If you wish to use Timer1 as a 20ms increment scheduler, you need to enable the interrupt and
//include in your code the ISR, something like:
...
/*If you wish to use the interrupt, you have to define the ISR. The code below averages
ADS_sample_max samples and filters the values that are below the threshould:
ISR (ADC_vect){
...
// Note that in current release PD4 is used as output to show ADC timing vs polling period
// You can connect an oscilloscope to it to see it
// If you wish to use it as input, you have to redefine it in the init section and remove XOR in the ADR ISR.
*/
...
// 2) The example is NOT practical - it is here just to show you how to operate the controls.
//  uint8_t dist_old=ADC_data.ADC3;
//  OCR0A=250; //hover, but not too high
//  OCR1B=D1B(127); //turning at lower speed speed
//  uint8_t a=0;
//  OCR1A=Servo_angle[a]; //servo to one side - change it to more appropriate angle 
...
//  }
//  
uint8_t distToWallStart = ADC_data.ADC3;
DELAY_ms(500);
OCR0A=250;
OCR1B=250;
uint8_t a, b, c = 0; //0 = Forward
b = 128; //128 = Right
c = 255; //256 = Backwards
...
DELAY_ms(500);
i -=15;
uint8_t currentDist = ADC_data.ADC3;
//Serial.print(currentDist);
if(currentDist > distToWallStart + DELTA) {
  break;
}
}
...
//====================================================================================|
//====================================================================================|
//===DO NOT CHANGE ANYTHING BELOW THESE LINES (unless you know what your are doing)===|
//====================================================================================|
//====================================================================================|
...
//    flags.ADC_ready=1;
      ...
      case 6: { 
        ...
        uint16_t PWM_temp;
        PWM_temp=(((uint16_t)V_batt-(uint16_t)BAT_warn)*255)/(255-(uint16_t)BAT_warn); 
...
// >>>>>>> UN-comment the line below to disable remote control commands processing once the "autonomous" mode was turned ON.
        ...
        if (!(ctrl[ctrl_count]&0b00010001)) PORTD&=~(1<<PD7); 
        ctrl_count++; 
        if (ctrl[ctrl_count-1]&0b01000100) OCR0A=ctrl[ctrl_count];  
          else OCR0A=0;
        ctrl_count++; 
        if (ctrl[ctrl_count-2]&0b10001000) OCR1B=D1B(ctrl[ctrl_count]+1); 
          else OCR1B=0;
        ctrl_count++;
...
//        if ((dist_raw>DIST_TH)&&flags.mode) flags.stop=1;
  ...
  else rx_buff=UDR0; //scrap the reading
}
...
ISR(TIMER1_CAPT_vect){ //system tick: 50Hz, 20ms
  ...
  time++;
  delay_ms+=20;
  if (!(ADCSRA&(1<<ADEN))) ADCSRA|=((1<<ADEN)|(1<<ADIE)|(1<<ADSC)); 
...
// ------ Watch dog init ---------------  
// Oct.29,2018: WDT has been disabled due to incompatibility with arduino bootloader
/*
  __asm__ __volatile__ ("wdr");
  MCUSR &= ~(1<<WDRF);
  WDTCSR|=(1<<WDCE) | (1<<WDE);
